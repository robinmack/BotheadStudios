...
- **`scripts/gpu-gate.sh` — something runs `gpu-verify` now.** The only check of the GPU granular path's
  ...
  0.1°–0.4° against real friction angles of 30–45°, a model deficiency docs/45 analyses and explicitly
  forbids "fixing" by tuning μ. The second one was invisible while nothing graded the tool's output.
...
- **An impact is sited where the trajectory crossed the surface, not where the step ended.**
  ...
  identical defect at planetary scale, where the speeds are higher. Both now bisect through one
  `flight::surface_crossing`, resolved to the surface field's own f32 precision. The trait's signature
  changed: `arrival(&self, body, from, to, dt) -> Option<flight::Met>`, where `Met` carries the site, the
  energy and the momentum — the environment prices the meeting because only it knows what was met.
- **Impact energy has ONE formula, and bare terrain is its immovable limit.**
  `flight::delivered(body, target_vel, target_mass_kg)` computes ½·μ·|Δv|² with the reduced mass, which is
  the law `interaction::detect_swept` already applied to every body-body collision; passing `None` for the
  target mass means matter too massive to move and reduces it exactly to the ½mv² and mv the terrain path
  wrote out separately. `Arrival` gained a `momentum` field, and every ground arrival now goes through the
  one `deposit_event` door — terrain voxels, cohesive-body parcels and debris grains in a single walk.
- **A declared solid body is hard matter a flight can meet.** `GroundAir` carries the scene's cohesive
  bodies, so a meteor meeting the ball is forecast on the swept segment by `interaction::detect_swept` (the
  one collision owner) and reported through the same door as a ground landing. Cohesive bodies also report
  through `Simulation::drawn()` as `crate::Drawn` now, so a scene no longer hand-writes `GpuParticle`
  literals to draw them. Integrated from Sean Reid's `upstream-5-ground-ball`; see
  `merge-reports/2026-07-25-sean-reid.md` for every decision made in his code.
...
- **The materialized site enters dynamics: the event's arriving energy moves and breaks the
  ...
  energy and the site's own clock.
