...
mod tests {
    ...
    fn the_split_conserves_all_five_quantities_exactly() {
        ...
        let n_parents = field.iter().filter(|p| region.contains(p.pos)).count();
        ...
        assert_eq!(
            split.particles.len() - split.fine_start,
            n_parents * SPLIT_CHILDREN
        );
        ...
        assert!(
            (a.mass - b.mass).abs() <= b.mass * 1.0e-6,
            "mass: {} vs {}",
            b.mass,
            a.mass
        );
    ...
    fn relax_releases_within_the_density_bound_across_a_basalt_iron_interface() {
        ...
        for c in children {
            let expected = if c.pos[0] < 0.0 {
                crate::gpu_sph::MAT_IRON
            } else {
                crate::gpu_sph::MAT_BASALT
            };
    ...
    fn a_second_rung_against_unbuffered_coarse_is_refused() {
        ...
        let rim = Region {
            center: Vec3::new(2.8, 0.0, 0.0),
            radius: 1.0,
        };
        match split_patch(&once.particles, &rim).map(|_| ()) {
            Err(Refusal::RatioExceeded {
                coarse_mass,
                parent_mass,
                ..
            }) => {
