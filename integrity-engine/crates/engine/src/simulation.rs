...
impl CohesiveBody {
    ...
    pub fn from_parcels(
        ...
        if particles.is_empty() {
            return Err(format!(
                "no parcels to build a cohesive {:?} body from",
                mat.id
            ));
        }
...
pub struct Simulation {
    ...
    /// a planet holds; only the environment below differs (docs/59). This subsumes the scene's own
    /// `meteors: Vec<Meteor>`: `Simulation::meteors()` now reads `Flight::bodies()`.
...
struct GroundAir<'a> {
    ...
    surface_pressure: f64,
    g: f64,
    /// **The declared solid bodies standing on this patch.** They are hard matter, so "has this path met
    /// hard matter" has to answer for them too — behind the same door as the terrain rather than beside
    /// it in the scene's own loop. Without this the body forecast could not survive at all: the scene no
    /// longer sees the per-substep segments, only [`crate::flight::Flight`] does.
    bodies: &'a [CohesiveBody],
...
impl GroundAir<'_> {
    ...
    /// asks `interaction::detect_swept`, the same owner that forecasts Theia into proto-Earth. A fast
    /// body cannot tunnel through a slow one between samples. On a contact that resolves matter it
    /// returns the EVENT: the site on the struck body's own skin, the striker's momentum, and the door's
    /// reduced-mass energy. It deposits nothing itself — delivery belongs to `deposit_event`, the same
    /// door a ground landing goes through.
    /// (Like the ground impact, the impactor's own matter joining the wreck is a flagged IOU.)
    ///
    /// Sean's `upstream-5-ground-ball` wrote this in the scene's meteor loop, which is where it had to
    /// live while the scene integrated the trajectory by hand. Same code, same physics; only its home
    /// changed, because `flight::Flight` now owns the trajectory and the scene never sees the swept
    /// segment. This is the seam the environment exists to be.
        ...
        for cb in self.bodies {
            ...
            let states = [
                crate::interaction::BodyState {
                    ...
                    mass_kg: mass,
                    radius_m: radius,
                    strength_pa: strength_of(cb.agg.material),
                    // A ball has no atmosphere of its own; this patch's air belongs to `GroundAir`.
                    air: None,
                },
                // The segment's own endpoints are exact; the velocity is the one the body carries at
                // contact (one gravity half-step from the pre-step value the scene loop used, which is
                // nothing beside an entry speed).
                crate::interaction::BodyState {
                    pos: from,
                    vel: body.vel,
            ...
            for h in crate::interaction::detect_swept(&states, &after_pos, &[true, true]) {
                if let crate::interaction::Response::ResolveMatter { .. } = h.response {
                    ...
                    return Some(crate::flight::Met {
                        at: after_pos[0] + n * (radius - cb.part_half),
                        // The collision owner's own reduced-mass energy — NOT recomputed here, so a
                        // meteor meeting a ball and two bodies meeting in orbit are priced by one law.
                        energy_j: h.energy_j,
                        momentum: v_meteor_rel * body.mass_kg,
                    });
                }
            }
        }
...
impl crate::flight::FlightEnvironment for GroundAir<'_> {
    ...
    fn arrival(
        ...
        // A declared body standing on the patch is hard matter, and it is in the way BEFORE the ground
        // is. Asked first, through the same door: detection picks the site, never the recipients.
        if let Some(met) = self.forecast_body_contact(body, from, to, dt) {
            return Some(met);
        }
        // THE shared ground height (`World::ground_height`) — the same surface the camera collides with.
        let ground_at = |p: glam::DVec3| self.world.ground_height(p.x as f32, p.z as f32) as f64;
        if to.y > ground_at(to) {
            return None;
        }
        // **The site is where the TRAJECTORY crossed the surface, not the post-step sample.** A meteor
        // moves metres per substep, so `to` can be metres underground and a buried site couples to the
        // wrong matter. (The swept body path above already had this honesty; the ground gets the same.)
        let at = crate::flight::surface_crossing(from, to, |p| p.y <= ground_at(p));
...
impl Simulation {
    ...
    pub fn drawn(&self) -> Vec<crate::Drawn> {
        ...
        // **A declared solid body is matter too** (docs/23): every lattice particle is drawn in its own
        // material's albedo and glows at its own temperature, by the same rule as a grain or a meteor.
        // There is no ball-specific renderer — Sean's comment on the code this replaces said exactly
        // that, and then had to hand-write `GpuParticle` literals in the scene to get it. Reporting the
        // body here is that same intent with the scene's copy removed: one mapping, in the engine.
        //
        // Bodies sit BETWEEN flight and grains in the budget order. A body is a declared object someone
        // is deliberately watching, so it should outlive settled grains when the list is cut, but matter
        // in flight still comes first for the reason above.
        out.extend(self.bodies.iter().flat_map(|b| {
            let (radius_m, material) = (b.part_half as f32, b.agg.material);
            b.agg
                .particles
                .iter()
                .zip(b.agg.temps.iter())
                .map(move |(p, t)| crate::Drawn {
                    pos: p.pos.as_vec3(),
                    vel: p.vel.as_vec3(),
                    radius_m,
                    material,
                    temp_k: *t,
                    // A body's particles carry no settle bookkeeping of their own; the scene uploaded
                    // 0.0 here. Kept faithful — this is a consolidation, not a behaviour edit.
                    resting: false,
                })
        }));
    ...
    fn fly_meteors(&mut self, dt: f32) -> usize {
        // **The engine flies them; this scene only says where the ground, the air and the bodies are.**
        // Everything that used to be written out here - drag, aeroheating, ablation, shedding the trail,
        // burning up, falling, arriving - is `flight::Flight::step`, the same code a planet runs. What is
        // left is the one thing that really is local: this patch's geometry, and what is standing on it.
        //
        // Sean's `upstream-5` kept the loop here and integrated the meteor by hand. That version was a
        // VACUUM ballistic path (`vel += g*dt`), so keeping it would have silently retired the atmosphere:
        // no drag, no aeroheating, no ablation, no trail. His three real additions - the body forecast,
        // the bisected site and momentum alongside energy - are kept, and have moved to where they can
        // still see the swept segment (`GroundAir::arrival`) now that the engine owns the trajectory.
        let env = GroundAir {
            world: &self.world,
            air: self.materials.iter().find(|mm| mm.id == "air"),
            surface_pressure: self.surface_pressure,
        ...
        for a in arrivals {
            ...
            created += self.deposit_event(a.at.as_vec3(), dir, a.momentum, a.energy_j);
        }
...
mod tests {
    ...
    /// **The ground-zero demo is ONE world definition** (docs/23, the demo the engine is aimed at;
    /// docs/59). `worlds/ground-zero/world.json` declares the whole cast in one file: the Sun, the
    ...
    #[test]
    fn the_ground_zero_world_executes_through_the_definition_paths_that_exist() {
        let shipped = std::fs::read_to_string(concat!(
            env!("CARGO_MANIFEST_DIR"),
        ...
        let def =
            crate::terra::world_def::World::parse(&shipped).expect("ground-zero world parses");
        assert_eq!(def.kind, "system", "the space band routes by type");
        let bodies = def
            .bodies
            .as_ref()
            .expect("a system world declares its cast");
        let roles: Vec<&str> = bodies.iter().map(|b| b.role.as_str()).collect();
        assert_eq!(
            roles,
            ["star", "planet", "moon"],
            "Sun, the shared Earth, one Luna"
        );
        let earth_def = &bodies[1];
        let luna = &bodies[2];
        assert_eq!(earth_def.profile.as_deref(), Some("earth"));
        assert_eq!(
            luna.profile.as_deref(),
            Some("moon"),
            "Luna is an instance of the defined Moon"
        );
        ...
        assert!(
            energy < 0.0,
            "the declared state is a bound orbit (E = {energy:.3e} J/kg)"
        );
        ...
        assert_eq!(
            sim.cohesive_bodies().len(),
            1,
            "the declared iron ball is built"
        );
        assert!(
            sim.cohesive_bodies()[0].agg.active_bonds() > 0,
            "the ball is a bonded solid"
        );
        assert_eq!(
        ...
        assert_eq!(
            sim.planet_radius_m(),
            earth.radius(),
            "the patch belongs to the one Earth"
        );
    }

    /// **Ground zero round-trips to the shared Earth's surface.** The ground-zero world places its
    ...
    #[test]
    fn the_ground_zero_site_round_trips_to_the_orbital_earths_surface() {
        let shipped = std::fs::read_to_string(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../web/public/worlds/ground-zero/world.json"
        ))
        ...
        let g = def
            .ground
            .as_ref()
            .expect("the demo declares its ground-zero placement");
        ...
        let placed = def
            ...
            .unwrap()
            .iter()
            .any(|b| b.role == "planet" && b.profile.as_deref() == Some(g.planet.as_str()));
        assert!(
            placed,
            "the ground's host body is the planet the system half flies"
        );
        ...
        assert!(
            crate::planet::is_land(g.lat, g.lon),
            "ground zero is a land site"
        );
    }
    ...
    fn a_meteors_energy_reaches_the_ball_through_the_shared_door() {
        ...
        sim.throw_meteor(Meteor {
            id: 0,
            pos: glam::DVec3::new(0.0, (ground + 80.0) as f64, 0.0),
            vel: glam::DVec3::new(0.0, -250.0, 0.0),
            mass_kg: 1200.0,
            material: crate::materials::index_of(&mats(), "iron"),
            radius_m: 0.33,
            temp_k: 288.0,
    ...
    fn one_impact_event_reaches_the_terrain_and_the_ball_through_one_door() {
        ...
        sim.throw_meteor(Meteor {
            id: 0,
            pos: glam::DVec3::new(0.0, (ground + 80.0) as f64, 0.0),
            vel: glam::DVec3::new(0.0, -500.0, 0.0),
            mass_kg: 2000.0,
            material: crate::materials::index_of(&mats(), "iron"),
            radius_m: 0.39,
            temp_k: 288.0,
    ...
    fn an_impact_event_heats_debris_grains_already_in_flight() {
        ...
        sim.throw_meteor(Meteor {
            id: 0,
            pos: glam::DVec3::new(0.0, (ground + 60.0) as f64, 0.0),
            vel: glam::DVec3::new(0.0, -400.0, 0.0),
            mass_kg: 1200.0,
            material: crate::materials::index_of(&mats(), "iron"),
            radius_m: 0.33,
            temp_k: 288.0,
    ...
    fn a_sufficient_meteor_shatters_the_ball_and_its_hottest_parcels_glow() {
        ...
        sim.throw_meteor(Meteor {
            id: 0,
            pos: glam::DVec3::new(0.0, (ground + 80.0) as f64, 0.0),
            vel: glam::DVec3::new(0.0, -17_000.0, 0.0),
            mass_kg: 1200.0,
            material: crate::materials::index_of(&mats(), "iron"),
            radius_m: 0.33,
            temp_k: 288.0,
        ...
        //
        // This assertion was `spread1 > 2.0 * spread0` on Sean's branch, where it passed — because there
        // the meteor flew in VACUUM. Through this patch's real air a 17 km/s body arrives with 1.645e11 J
        // instead of 1.720e11 (2.2% of its speed lost over the last 80 m), and the scatter falls to 1.51x.
        // Verified by ablation: with `air_at` forced to `None` the original 2.0x bar passes again, so the
        // atmosphere is the whole of the difference. The physics got MORE honest and the threshold went red.
        //
        // Retuning the multiple would only move an arbitrary constant to wherever today's number sits, and
        // it would go red again at the next honest change. So the bound comes from the BODY's own geometry
        // instead: the cloud has to have grown by more than one lattice half-spacing, which is the distance
        // below which the parcels are still sitting in the positions their bonds held them in. Above it,
        // they have measurably rearranged and it is no longer that ball.
        //
        // ("Still growing a second later" was tried first and is FALSE, which is itself worth knowing:
        // measured 1.91 m -> 1.91 m. The debris lands. Under real gravity onto real terrain a shattered
        // pile settles rather than dispersing for ever, so "the parcels are unbound" is not what this
        // event produces and must not be what the test claims.)
        let lattice = sim.cohesive_bodies()[0].part_half;
        assert!(
            spread1 - spread0 > lattice,
            "the parcels rearranged past their bonded spacing - it is no longer a ball \
             ({spread0:.2} m -> {spread1:.2} m rms, lattice half-spacing {lattice:.2} m)"
        );
    ...
    fn an_insufficient_meteor_displaces_the_ball_and_it_survives() {
        ...
        sim.throw_meteor(Meteor {
            id: 0,
            pos: glam::DVec3::new(0.0, (ground + 80.0) as f64, 0.0),
            vel: glam::DVec3::new(0.0, -60.0, 0.0),
            mass_kg: 300.0,
            material: crate::materials::index_of(&mats(), "iron"),
            radius_m: 0.21,
            temp_k: 288.0,
