//! **The declared site: camera-driven materialization of ground zero** (docs/62 order-of-work
...
//!   PARTICLE-form matter; meshed, standable ground at the site is docs/62 item 4.
//! - The patch budget is a declared compute statement (docs/62 names this IOU): a bowl of
...
/// Fine particles the materialized site may spend, riding the same statement (docs/62's open
...
/// the world definition and the one shared body (docs/62 "one Earth": the strata, gravity and
...
impl SiteSpec {
    ...
    pub fn from_ground_world(w: &World) -> Result<SiteSpec, String> {
        let g = w.ground.as_ref().ok_or_else(|| {
            format!(
                "world {:?} declares no ground block; no site to arm",
                w.name
            )
        })?;
    ...
    pub fn finest_child_extent_m(&self, mats: &[Material]) -> Option<f64> {
        match self.ball.as_ref() {
            Some(b) => {
                ...
                Some(coarse_particle_extent_m(
                    m_ball / refine::LEVEL_MASS_RATIO,
                    rho,
                ))
            }
...
pub fn sample_hand_down(
    ...
    let v_bulk = cover.iter().fold(DVec3::ZERO, |a, p| {
        a + DVec3::from(Vec3::from(p.vel).as_dvec3()) * p.mass as f64
    }) / m_tot;
    let extent = cover
        .iter()
        .map(|p| coarse_particle_extent_m(p.mass as f64, p.rho.max(1.0) as f64))
    ...
    let peak = cover
        ...
        .fold(0.0, f64::max);
    if peak >= v_q {
        return Err(SiteRefusal::MidEvent {
            peak_speed_ms: peak,
            quiescent_speed_ms: v_q,
        });
    }
    let u = cover
        .iter()
        .map(|p| p.mass as f64 * p.u as f64)
        .sum::<f64>()
        / m_tot;
    Ok(HandDown::Sampled {
        u_j_kg: u,
...
pub fn materialize_site(
    ...
    let lookup = |id: &str| -> Result<(usize, f64, f64), SiteRefusal> {
        let idx =
            mats.iter()
                .position(|m| m.id == id)
                .ok_or_else(|| SiteRefusal::UnknownMaterial {
                    material: id.into(),
                })?;
        let c = mats[idx]
            .specific_heat()
            .ok_or_else(|| SiteRefusal::UnsourcedHeat {
                material: id.into(),
            })?;
    ...
    let top_b = crate::world::terrain_height_with(&spec.surface, vx_b as f32, vz_b as f32) as f64;
    ...
    // the materialization.
    let (particles, fine_start, mut ledger, release) = match refine::refine_patch(&parents, &region)
    {
        Ok(t) => {
    ...
    if ball.is_some() {
        ...
        let b_region = refine::Region {
            center: Vec3::from(parent.pos),
            radius: parent.h,
        };
...
// -----------------------------------------------------------------------------------------------
// The mid-event hand-down (docs/62 "The hand-down, made concrete", decision 2): during a live
...
impl SiteFrame {
    ...
    pub fn field_vec_to_site(&self, v: DVec3) -> [f32; 3] {
        [
            v.dot(self.east) as f32,
            v.dot(self.up) as f32,
            v.dot(self.north) as f32,
        ]
    }
}
...
/// collapse until the N-material upload reaches the guards (docs/62).
pub fn resample_guards(
    ...
    let near: Vec<&SphParticle> = field
        ...
        .filter(|p| {
            let d =
                DVec3::new(p.pos[0] as f64, p.pos[1] as f64, p.pos[2] as f64) - frame.origin_rel_m;
...
impl EventWindow {
    ...
    /// coarse field's collapsed EOS material set (docs/62 names this the hand-down's fidelity
...
// -----------------------------------------------------------------------------------------------
// The site in DYNAMICS (docs/62, the remaining core of the hand-down): once the rung RELEASES the
...
impl SiteDynamics {
    ...
    pub fn new(
        ...
        let build = |idxs: &[usize], spacing: f64| -> Result<SiteBody, SiteRefusal> {
            ...
            let m = mats.get(mat).ok_or_else(|| SiteRefusal::UnknownMaterial {
                material: format!("#{mat}"),
            })?;
            let c_heat = m
                .specific_heat()
            ...
            .map_err(|_| SiteRefusal::UnknownMaterial {
                material: m.id.clone(),
            })?;
...
mod tests {
    ...
    fn the_view_threshold_is_where_one_coarse_particle_subtends_the_budget() {
        ...
        assert!(
            (s - 9.50e5).abs() < 5.0e3,
            "hand-computed quantum: got {s:.3e}"
        );
        ...
        assert!(
            (8.0e8..1.2e9).contains(&d),
            "threshold ~9.5e8 m, got {d:.3e}"
        );
        ...
        assert!(
            (ratio - 13.0f64.cbrt()).abs() < 1.0e-9,
            "one rung = 13^(1/3) closer"
        );
        ...
        assert!(
            (measured_coarse_extent_m(&field) - 2.0).abs() < 1.0e-6,
            "max share, not mean"
        );
    }
    ...
    fn the_trigger_fires_descending_and_rearms_ascending() {
        ...
        assert_eq!(
            t.observe(1.2e9, at),
            None,
            "folded and outside: armed, silent"
        );
    ...
    fn the_materialized_site_conserves_through_the_refine_audit() {
        ...
        let ball = spec
            .ball
            .as_ref()
            .expect("the shipped world declares the ball");
        ...
        let spread = kids.iter().map(|p| Vec3::from(p.pos)).fold(
        ...
        assert!(
            !patch.is_empty(),
            "a terrain patch materializes alongside the ball"
        );
        for p in patch {
            assert!(
        ...
        assert!(
            (l.after_relax.kinetic - l.before.kinetic).abs() <= l.before.kinetic.max(1.0) * 1.0e-6
        );
        assert!(
        ...
        let release_line = match site.release {
            ...
            SiteRelease::Unreleased {
                ...
                assert!(
                    achieved.is_finite() && achieved > bound,
                    "a stated, real residual"
                );
                assert_eq!(
    ...
    fn the_site_column_agrees_with_the_generated_ground() {
        ...
        for p in patch {
            ...
            let Some(mat) = world.material_at(vx, vy, vz) else {
                continue;
            };
        ...
        assert!(
            compared > 10,
            "the comparison must actually bite (compared {compared})"
        );
    }
    ...
    fn the_hand_down_samples_quiet_fields_and_refuses_mid_event() {
        ...
        match sample_hand_down(&quiet, site, g) {
            Ok(HandDown::Sampled {
                ...
                assert!(
                    (4000.0..4700.0).contains(&quiescent_speed_ms),
                    "v_q from the quantum"
                );
            }
        ...
        let comoving = vec![
            mk([6000.0, 1.0, 0.0], 2.0e5),
            mk([6000.0, -1.0, 0.0], 2.0e5),
        ];
        assert!(
            sample_hand_down(&comoving, site, g).is_ok(),
            "bulk motion is not an event"
        );
        ...
        let spec = SiteSpec::from_ground_world(&shipped_ground_world()).expect("spec");
        let mats = materials::load();
        let hand = HandDown::Sampled {
            u_j_kg: 7.7e5,
            peak_speed_ms: 1.0,
            quiescent_speed_ms: 4000.0,
        };
        let site = materialize_site(&spec, &hand, &mats).expect("materializes");
        for p in &site.particles {
            assert!(
                (p.u - 7.7e5).abs() < 1.0,
                "the sampled u carries down to every particle"
            );
        }
    }

    /// **The guards ARE the coarse field at the boundary, and the window books the event.**
    /// The docs/62 hand-down made concrete, decision 2: during a live event the guard band
    ...
    fn the_guards_carry_the_coarse_field_and_the_window_books_the_event() {
        ...
        let m_guard: f64 = site.particles[..site.fine_start]
            .iter()
            .map(|p| p.mass as f64)
            .sum();
        assert!(
            site.fine_start > 0,
            "the site carries a guard band to be the boundary"
        );
        ...
        assert!(
            window.peak_speed_ms() >= v_len * (1.0 - 1.0e-6),
            "the peak stays booked"
        );
        ...
        for (a, b) in before[site.fine_start..]
            .iter()
            .zip(&site.particles[site.fine_start..])
        {
            assert_eq!(
                a.vel, b.vel,
                "the RESAMPLE alone never steps the fine patch"
            );
            assert_eq!(a.u, b.u);
        ...
        assert!(
            line.contains("EOS"),
            "the guard band's EOS collapse is a stated IOU: {line}"
        );
    ...
    /// **The released site enters dynamics and, quiescent, comes to rest** (docs/62, the
    ...
    fn the_released_site_steps_quiescently_and_the_ball_falls_to_rest() {
        ...
        assert_eq!(
            dyn_.ball_verdict(),
            Some("intact"),
            "the declared ball starts whole"
        );
        ...
        assert!(
            y1 < y0 - 0.5,
            "gravity pulls the ball down without help ({y0:.2} -> {y1:.2})"
        );
        assert!(
            y1 > -8.0,
            "and the patch/bowl stops it - it does not fall through the ground"
        );
        ...
        assert!(
            gauge.settled(g),
            "a quiescent site settles at the docs/61 criterion"
        );
    ...
    /// ball** (docs/23's sentence at the site; docs/62 decision 2 completed). The guards mirror
    ...
    fn a_landed_drops_boundary_energy_shatters_the_sites_ball_by_classify() {
        ...
        let m_fine: f64 = site.particles[site.fine_start..]
            .iter()
            .map(|p| p.mass as f64)
            .sum();
        ...
        assert!(
            vap + melt > 0,
            "at these densities some matter melts or vaporizes"
        );
        ...
        let spread = |site: &MaterializedSite| {
            ...
            ps.iter()
                .map(|p| (Vec3::from(p.pos) - com).length())
                .fold(0.0f32, f32::max)
        };
        let s0 = spread(&site);
        let mut t2 = 0.0;
        ...
        assert!(
            matches!(
                fold_site(&site, &gauge, g),
                Err(SiteRefusal::NotSettled { .. })
            ),
    ...
    fn an_unreleased_site_is_refused_dynamics_by_the_standing_gate() {
        ...
        let mats = materials::load();
        let mut site = materialize_site(&spec, &HandDown::Declared, &mats).expect("materializes");
        site.release = SiteRelease::Unreleased {
            achieved: 4.8e-2,
            bound: 5.0e-3,
            iterations: 400,
        };
        match SiteDynamics::new(&site, &spec, &mats) {
            ...
            other => panic!(
                "an unreleased site must not enter dynamics, got {:?}",
                other.is_ok()
            ),
        }
    }
