...
mod tests {
    ...
    fn the_batch_rung_folds_a_settled_field_and_keeps_the_remainder() {
        ...
        assert_eq!(
            folded.voxels, 1,
            "1.6 quanta afford exactly ONE whole voxel"
        );
        assert_eq!(w.solid_count(), before + 1);
        assert_eq!(
            w.material_at(4, 1, 4),
            Some(gravel),
            "gravel comes back as gravel"
        );
        assert_eq!(
            sim.particle_count(),
            1,
            "the 0.6-quantum remainder stays a particle"
        );
        ...
        assert!(
            sim.take_dirty(),
            "a fold changes the ground, so the surface must remesh"
        );
    }
