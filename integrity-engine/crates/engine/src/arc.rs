...
impl ArcSpan {
    ...
    pub fn derive(
        ...
        ArcSpan {
            d_floor_m: crate::site::view_resolution_distance(
                finest_quantum_m,
...
mod tests {
    ...
    fn shipped_span() -> (ArcSpan, SiteSpec, Vec<materials::Material>) {
        ...
        let span = ArcSpan::derive(
            ...
            w.arc
                .as_ref()
                .map(|a| a.octave_s)
                .expect("the shipped world declares arc pacing"),
        );
        (span, spec, mats)
    }
    ...
    fn the_span_derives_floor_from_the_finest_quantum_and_top_clears_the_fold() {
        ...
        let q = spec
            .finest_child_extent_m(&mats)
            .expect("ball world has a quantum");
        assert!(
            (q - 1.37).abs() < 0.02,
            "hand-computed child quantum, got {q:.3}"
        );
        assert!(
        ...
        let threshold = site::view_resolution_distance(spec.declared_coarse_extent_m(), theta);
        assert!(
            span.d_top_m >= WHOLE_ORBIT_MARGIN * threshold,
            "top clears the fold"
        );
        assert!(
            span.d_top_m > threshold && span.d_floor_m < threshold,
            "the arc spans the crossing"
        );
    ...
    fn the_pacing_compresses_time_in_proportion_to_camera_distance() {
        ...
        assert_eq!(
            span.time_scale(span.d_top_m),
            span.declared_scale,
            "declared at the top"
        );
        assert_eq!(
            span.time_scale(span.d_floor_m),
            1.0,
            "real time at the floor"
        );
        ...
        assert!(
            (k1 - k2).abs() <= k1 * 1.0e-12,
            "S ∝ d holds: {k1:.6e} vs {k2:.6e}"
        );
        ...
        assert!(
            span.time_scale(knee * 1.01) < 1.02,
            "no jump above the knee"
        );
    ...
    fn the_arc_pose_is_continuous_surface_to_celestial_and_back() {
        ...
        for descending in [true, false] {
            ...
            let from = if descending {
                slerp_dir(
                    site_body,
                    -site_body + DVec3::new(0.1, 0.2, 0.0),
                    2.6 / std::f64::consts::PI,
                )
            } else {
                site_body
            };
            ...
            loop {
                ...
                assert!(
                    (e - prev_eye).length()
                        <= step_bound(d.max(prev_eye.length() - r), alpha0, leg.d_start_m),
    ...
    fn the_approach_pan_ends_at_the_planet_and_descends_overhead() {
        ...
        let d0 = span.d_top_m;
        assert!(
            (approach_angle(alpha, d0, d0, r) - alpha).abs() < 1.0e-12,
            "full offset at start"
        );
        assert!(
            approach_angle(alpha, r, d0, r) == 0.0,
            "zero at one planet radius"
        );
        assert!(
            approach_angle(alpha, span.d_floor_m, d0, r) == 0.0,
            "overhead below it"
        );
        ...
        for t in [0.25, 0.5, 0.75] {
            assert!(
                (slerp_dir(a, b, t).length() - 1.0).abs() < 1.0e-12,
                "unit mid-path"
            );
            assert!(
                (slerp_dir(a, -a, t).length() - 1.0).abs() < 1.0e-9,
                "antipodal stays unit"
            );
        }
    }
