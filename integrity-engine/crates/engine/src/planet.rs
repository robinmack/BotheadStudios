...
impl LayeredBody {
    ...
    pub fn surface_strata(
        &self,
        lat_deg: f64,
        lon_deg: f64,
    ) -> Vec<crate::terra::world_def::Stratum> {
...
mod surface_strata_tests {
    ...
    fn a_patch_column_derives_from_the_bodys_own_layers() {
        ...
        assert_eq!(
            strata.last().unwrap().thickness_m,
            None,
            "the core fills the rest"
        );
    }
    ...
    fn the_skin_follows_the_bodys_surface_at_the_site() {
        ...
        assert_eq!(
            ocean[0].material, "basalt",
            "no biosphere skin on the sea floor"
        );
        let m = moon();
        assert_eq!(
