...
pub fn solve_drop_window(
    ...
    let mut eval = |t: f64| -> Option<(f64, DVec3, f64)> {
        let (dir, t_impact) =
            from_rest_fall_contact(bodies, planet, drop, r_contact, dt, t, horizon)?;
        let err = wrap_pi(azimuth_about(spin.axis, dir) - (site_az0 + spin.omega_rad_s * t_impact));
...
mod tests {
    ...
    fn simulate_release_and_measure_miss(
        ...
        loop {
            ...
            if let Some(f) = crate::orbit::swept_first_contact(rel_old, rel_new, r_contact) {
                ...
                let site_az =
                    azimuth_about(spin.axis, crate::geo::dir_from_lat_lon(site_lat, site_lon))
                        + spin.angle_rad
    ...
    fn a_far_side_site_waits_half_a_day_with_the_same_trajectory() {
        ...
        println!(
            "inertial impact-azimuth drift over the wait: {:.2}°",
            drift.to_degrees()
        );
        assert!(
