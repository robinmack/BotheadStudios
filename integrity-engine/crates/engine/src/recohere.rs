...
mod tests {
    ...
    fn a_settled_mound_recoheres_conserving_mass_and_material() {
        ...
        assert_eq!(
            w.solid_count() - solids_before,
            6,
            "the world gained exactly those voxels"
        );
        ...
        assert_eq!(
            r.remainder.len(),
            1,
            "only the sub-quantum mass stays a particle"
        );
    ...
    fn the_crossing_measures_the_binned_kinetic_energy_and_carried_heat() {
        ...
        let c = mats[gravel]
            .specific_heat()
            .expect("gravel thermal data is sourced");
        let t_ref = crate::matter::REF_TEMP_K as f64;
        ...
        assert!(
            (r.remainder[0].temp_k - 340.0).abs() < 1e-3,
            "the remainder keeps its own heat"
        );
    }
    ...
    fn a_still_moving_region_is_refused_and_the_world_is_untouched() {
        ...
        assert_eq!(
            w.solid_count(),
            solids_before,
            "refusal must not touch the world"
        );
    ...
    fn the_settle_criterion_is_seconds_of_quiet_not_frames() {
        ...
        assert!(
            (t_q - (2.0f32 / 9.81).sqrt()).abs() < 1e-6,
            "one cell free-fall time at Earth g"
        );
        ...
        assert!(
            coarse.settled(g) && fine.settled(g),
            "the step size must not decide settling"
        );
        ...
        assert!(
            !short.settled(g),
            "quiet shorter than the cell dynamical time is not settled"
        );
        ...
        assert!(
            !jolted.settled(g),
            "one loud observation must restart the sustained window"
        );
