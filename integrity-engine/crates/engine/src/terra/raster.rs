...
mod tests {
    ...
    fn texel_arc_is_the_coarser_axis_of_the_raster() {
        ...
        assert!(
            (t - std::f64::consts::TAU * r / 2048.0).abs() < 1e-9,
            "2:1 raster: both axes agree, got {t}"
        );
        assert!(
            (19_000.0..20_000.0).contains(&t),
            "the shipped raster's texel is ~19.5 km, got {t}"
        );
        ...
        assert!(
            (squat.texel_arc_m(r) - 2.0 * t).abs() < 1e-6,
            "half the rows, twice the texel"
        );
