# docs/62 — The zoom: celestial to local, one event

*Renumbered from 59 on integration: `docs/59-orbit-to-ground-swarm.md` (the flagship acceptance test)
already held that number on `main`. The two are the same journey seen from different ends — 59 states the
staged plan and what a viewer must SEE, this one states the method and order of work for materializing the
site. Read them together; neither supersedes the other.*
