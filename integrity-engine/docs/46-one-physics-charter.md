...
| 21 | **A body ablating in air is treated as isothermal.** `atmospheric_step` heats the body's whole mass at its bulk heat capacity, so thermal response scales with volume when real ablation is a SURFACE process — a thin skin melts and sheds while the interior stays cold | measured 2026-07-24: a 0.5 m iron body at 20 km/s barely warms through the entire descent, while a 1 cm grain reaches iron's boiling point in milliseconds. That the model gets the *observable* right (small meteoroids burn up, iron meteorites land) is not evidence the mechanism is right — it is right only while the body is thinner than its thermal skin depth, √(αt) ≈ 1.5 cm for iron over ten seconds | this doc — the resolved computation is a temperature PROFILE through the body; flagged in `atmospheric_step` and in the entry test that measured it |
