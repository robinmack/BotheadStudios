...
| 8 | ~~**The honest footprint is computed, then discarded.**~~ **RETIRED 2026-07-23 (code deleted, not fixed).** The `crater_radius`-clipped-by-`MATERIALIZE_CAP` path lived in the terrain scene deleted in docs/50; `MATERIALIZE_CAP` no longer exists in the source. The underlying question — resolve the energy-sized footprint, not a fixed patch — is now carried by ledger note "resolution EXTENT is unbuilt" (intro) and by `resolution::admission_depth` (tested, unwired: `ResolutionField::update` still hardcodes `necessity_depth = 1.0`) | was: `lib.rs` clip, since removed | **docs/44** — the honest mechanism to wire when a quasi-static/footprint scene needs it |
