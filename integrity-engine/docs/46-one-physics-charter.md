...
| 14 | **A scene is engine code, not data.** Robin's requirement: a scene should be object/assembly definitions, coordinates and materials — and "should NOT require special mods of the engine itself". Every scene is instead a `#[wasm_bindgen]` struct INSIDE `crates/engine`, owning its own pipelines, uniforms and render loop | measured by deletion, 2026-07-21: removing ONE scene (terrain `Engine`) took **1,516 lines out of `lib.rs`**, a symbol out of the crate's public API (`pub use app::Engine`), and a build entry point. Adding one costs the same in reverse. `OrbitDemo` and `Terra` are identical in shape | **docs/50** — the next increment of docs/33's convergence; docs/43 built the worlds-as-data schema, the SCENE is still code |
